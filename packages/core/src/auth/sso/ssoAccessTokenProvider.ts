...
import { asStringifiedStack, withTelemetryContext } from '../../shared/telemetry/spans'
...
export abstract class SsoAccessTokenProvider {
    ...
    public async invalidate(reason: string): Promise<void> {
        ...
        // most of the time cache is deleted on cache expiration, this is infrequent.
        // Any other frequent scenarios that are not explicit deletion by the user are probably bugs.
        await telemetry.auth_modifyConnection.run(
            async (span) => {
                span.record({
                    source: asStringifiedStack(telemetry.getFunctionStack()),
                    action: 'invalidateSsoCache',
